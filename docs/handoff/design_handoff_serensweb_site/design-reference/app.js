...
(function () {
  ...
  // expose for the design-only Tweaks panel so the two stay in sync
  window.__serensApplyAccent = applyAccent;
  window.__serensSavedAccent = savedAccent;

  /* ---- comparison-canvas helper: auto-scroll to a region ---- */
  const scrollTo = params.get('to');
  if (scrollTo === 'footer') {
    window.addEventListener('load', () => {
      const f = document.querySelector('.footer');
      if (f) window.scrollTo({ top: f.offsetTop - 40 });
    });
  }

  /* ---- transition feature-probe ----
     Some environments (offscreen/throttled iframes) freeze CSS transitions,
     which would leave opacity:0 reveal content stuck hidden. Detect that and
     fall back to no-anim (everything visible, no transition) so the page is
     never blank. Real foreground browsers pass the probe and keep animations. */
  (function probeTransitions() {
    try {
      ...
      setTimeout(() => {
        ...
        p.remove();
        if (window.__serensRevealCheck) window.__serensRevealCheck();
      }, 120);
    } catch (e) { /* ignore */ }
  })();
  ...
  /* ---- scroll reveal (rect-based — robust across environments + viewport toggle) ---- */
  ...
  // expose so the viewport toggle can re-check after layout reflow
  window.__serensRevealCheck = revealCheck;
  ...
  const PROJECTS = [
    ...
    { viz: 'viz-b', cat: 'AI Web App', title: 'Brief: AI writing copilot', blurb: 'A focused writing tool with an LLM assistant that drafts, rewrites and structures copy in real time.',
      ...
      metrics: [['12k', 'Users'], ['1.4s', 'TTFB'], ['4.8★', 'Rating']],
